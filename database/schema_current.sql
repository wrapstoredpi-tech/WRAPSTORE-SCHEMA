-- ============================================================
-- WRAPSTORE — CANONICAL CURRENT SCHEMA
-- database/schema_current.sql
--
-- This file reflects the EXACT production state as of 2026-09-24.
-- It is the single source of truth, replacing:
--   schema.sql (stale — Stage 1 only, RLS enabled, Stage 2 commented out)
--   schema_stage2.sql (stale — RLS enabled, missing WhatsApp columns)
--   schema_stage3.sql (stale — RLS enabled)
--   full_schema.sql (stale — RLS enabled, missing channel columns)
--   schema_stage0.sql (supplemental — system_connection_test + storage)
--   omnichannel_migration.sql (additive — channel tracking not yet applied)
--   create_super_admin.sql (unchanged — operational script, not schema)
--   storage_setup.sql (supplemental — duplicated by schema_stage0.sql)
--
-- KEY PRODUCTION FACTS:
--   • RLS is DISABLED on all tables (per fix_rls_policies.sql intent)
--   • product_type CHECK is 'general' (not the 3-value enum in schema.sql)
--   • full_schema.sql is the closest prior reference; this file extends it
--   • omnichannel_migration.sql columns (channel, reserved_stock, etc.) are
--     NOT applied yet — they belong in migration_channel_tracking.sql
-- ============================================================
...
-- ============================================================
-- 4. PRODUCTS
-- NOTE: product_type is 'general' (open TEXT), NOT the 3-value
-- enum ('iphone_case','samsung_case','mobile_sticker') in schema.sql.
-- full_schema.sql has DEFAULT 'general' which matches production.
-- ============================================================
...
-- ============================================================
-- 8. INVOICES
-- NOTE: Includes WhatsApp columns from schema_stage3.sql.
-- Does NOT yet include channel / order_status columns —
-- those are in migration_channel_tracking.sql.
-- ============================================================
...
-- ============================================================
-- 12. SYSTEM CONNECTION TEST (from schema_stage0.sql)
-- ============================================================
...
ALTER TABLE system_connection_test DISABLE ROW LEVEL SECURITY;
...
-- NOTE: schema_stage2.sql reads invoice_prefix from store_settings.
-- full_schema.sql uses a hardcoded 'WS-INV-'. The store_settings approach
-- is more correct; retained here.
...
CREATE TRIGGER trigger_subcategories_updated_at
  BEFORE UPDATE ON subcategories FOR EACH ROW EXECUTE FUNCTION update_updated_at();
...
-- ============================================================
-- 14. HELPER FUNCTION (used by RLS when/if re-enabled later)
-- ============================================================
